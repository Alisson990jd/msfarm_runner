...
MAX_CONCURRENT = 4  # Execuções simultâneas
...
class GitHubManager:
    """Gerencia sincronização com GitHub"""
...
class EmailManager:
    ...
    def get_pending_today(self):
        """Retorna emails que ainda não foram executados hoje"""
        today = datetime.now().strftime('%Y-%m-%d')
        pending = []
        ...
        for email in self.emails:
            ...
            # Verificar se já executou hoje com sucesso
...
class ExecutionManager:
    """Gerencia a execução dos scripts"""
    ...
    async def get_next_display(self):
        """Obtém próximo número de display disponível"""
        async with self.display_lock:
            self.display_counter += 1
            return self.display_counter + 99  # Começa do display :100
    
    async def execute_email(self, email, display_num):
        """Executa o script para um email específico"""
        start_time = datetime.now()
        
        try:
            await self.send_notification(f"🔄 **Iniciando execução**\n📧 Email: `{email}`\n🖥️ Display: :{display_num}")
            
            # Criar ambiente virtual único
            venv_dir = os.path.join(SCRIPT_DIR, f".venv_{email.replace('@', '_').replace('.', '_')}_{display_num}")
    ...
    async def run_daily_batch(self):
        """Executa batch diário de todos os emails pendentes"""
        await self.email_manager.sync_from_github()
        pending = self.email_manager.get_pending_today()
        
        if not pending:
            await self.send_notification("ℹ️ **Nenhum email pendente para execução hoje!**")
            return
        
        await self.send_notification(
            f"🚀 **Iniciando batch diário**\n"
            f"📊 Total de emails: {len(pending)}\n"
            f"⚡ Execuções paralelas: {MAX_CONCURRENT}"
        )
...
async def on_ready():
    ...
    if channel:
        embed = discord.Embed(
            title="🧠 Cérebro Central Online!",
            description="Sistema de automação iniciado com sucesso.",
            color=discord.Color.green(),
            timestamp=datetime.now()
        )
        embed.add_field(name="📧 Emails carregados", value=str(len(email_manager.emails)), inline=True)
        embed.add_field(name="⚡ Execuções paralelas", value=str(MAX_CONCURRENT), inline=True)
        embed.add_field(
            ...
            value=(
                ...
                "`!stop` - Parar execuções"
            ),
            inline=False
        )
        await channel.send(embed=embed)
    
    # Iniciar task de execução diária
    daily_task.start()
...
async def add_email(ctx, email: str):
    ...
    if await email_manager.add_email(email):
        ...
        await ctx.send(f"✅ Email adicionado: `{email}`\n📊 Total de emails: {len(email_manager.emails)}")
    else:
        await ctx.send(f"⚠️ Email já existe: `{email}`")
...
async def remove_email(ctx, email: str):
    ...
    if await email_manager.remove_email(email):
        await email_manager.sync_to_github(update_emails=True, update_usage=False)
        await ctx.send(f"✅ Email removido: `{email}`\n📊 Total de emails: {len(email_manager.emails)}")
    else:
        await ctx.send(f"⚠️ Email não encontrado: `{email}`")
...
async def list_emails(ctx):
    ...
    if not email_manager.emails:
        await ctx.send("📭 Nenhum email cadastrado.")
        return
...
async def status(ctx):
    ...
    embed.add_field(name="📧 Total de emails", value=str(len(email_manager.emails)), inline=True)
    embed.add_field(name="⏳ Pendentes hoje", value=str(len(pending)), inline=True)
    embed.add_field(name="✅ Executados hoje", value=f"{success_today}/{executed_today}", inline=True)
    embed.add_field(name="🔄 Em execução", value=str(len(execution_manager.active_tasks)), inline=True)
    embed.add_field(name="⚡ Status", value="🟢 Ativo" if execution_manager.running else "🔵 Idle", inline=True)
...
async def pending(ctx):
    ...
    if not pending:
        await ctx.send("✅ Todos os emails já foram executados hoje!")
        return
    
    emails_text = "\n".join([f"• `{e}`" for e in pending[:50]])
    
    embed = discord.Embed(
        title="⏳ Emails Pendentes Hoje",
        description=emails_text[:4000],
...
async def run_manual(ctx):
    ...
    if execution_manager.running:
        await ctx.send("⚠️ Já existe uma execução em andamento!")
        return
    
    await ctx.send("🚀 Iniciando execução manual...")
...
async def sync(ctx):
    ...
    await ctx.send("🔄 Sincronizando com GitHub...")
    
    if await email_manager.sync_from_github():
        await ctx.send(f"✅ Sincronizado! {len(email_manager.emails)} emails carregados.")
    else:
        await ctx.send("⚠️ Erro ao sincronizar, usando dados locais.")
...
async def stop(ctx):
    ...
    if not execution_manager.active_tasks:
        await ctx.send("ℹ️ Nenhuma execução em andamento.")
        return
    ...
    await ctx.send(f"🛑 {len(execution_manager.active_tasks)} execuções canceladas.")
...
async def history(ctx, email: str = None):
    ...
    if email:
        usage = email_manager.usage.get(email, {})
        if not usage:
            await ctx.send(f"📭 Nenhum histórico para `{email}`")
            return
        ...
        embed = discord.Embed(
            title=f"📜 Histórico: {email}",
            description=history_text or "Sem execuções",
            color=discord.Color.blue()
        )
        embed.add_field(name="✅ Sucessos", value=str(usage.get("total_success", 0)), inline=True)
        embed.add_field(name="❌ Falhas", value=str(usage.get("total_failed", 0)), inline=True)
        await ctx.send(embed=embed)
    else:
        await ctx.send("❓ Use: `!history email@email.com`")


@tasks.loop(hours=24)
async def daily_task():
    """Task de execução diária automática"""
    ...
    logger.info(f"Próxima execução diária em {wait_seconds/3600:.2f} horas")
    
    await asyncio.sleep(wait_seconds)
    
    channel = bot.get_channel(CHANNEL_ID)
    if channel:
        await channel.send("⏰ **Execução diária automática iniciando...**")
